...
const ViewApplicants = () => {
  ...
  const handleStatusUpdate = async (newStatus) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/applications/${selectedApplicant.applicationId}/status`, {
        method: 'PUT',
        headers: {
      ...
      if (res.ok) {
        ...
        setStatusAnnouncement(`Status for ${updated.candidateName} set to ${newStatus}`);
      }
    } catch (err) {
  ...
  const handleScheduleInterview = async (e) => {
    e.preventDefault();
    if (!selectedApplicant || !interviewDate || !token) return;
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/interviews`, {
        ...
        body: JSON.stringify({
          ...
          meetingLink: meetingLink || 'https://meet.google.com/interview-session',
